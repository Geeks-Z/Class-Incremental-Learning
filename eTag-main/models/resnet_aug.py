...
__all__ = ['resnet18_cifar', 'resnet18_cifar_aux', 'resnet34_cifar',
           'resnet34_cifar_aux', 'resnet50_cifar','resnet50_cifar_aux']
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.embed = nn.Linear(512 * block.expansion, num_classes)
        # self.embed = nn.Sequential(nn.Linear(512 * block.expansion, 512 * block.expansion),
        #                            nn.LeakyReLU(0.2),
...
class resnet18_pretrain(nn.Module):
    ...
    def __init__(self, args):
        ...
        #     nn.Linear(self.feat_dim, self.feat_dim),
        #     nn.LeakyReLU(),
        #     nn.Linear(self.feat_dim, self.num_class),
            # nn.Sigmoid(),
        # )
        self.embed = nn.Sequential(
            nn.Linear(self.feat_dim, self.num_class)
        ...
        self.embed.apply(init_normal)

    def forward(self, x):
        feas = self.backbone(x)
        # torch.flatten(feas, 1)
        y_hat = self.embed(feas)
        return y_hat
...
if __name__ == '__main__':
    ...
    class params(object):
        def __init__(self):
            ...
            self.num_class = 100
    args = params()
    net = resnet18_pretrain(args)
    y_hat = net(x)
    print(y_hat.shape)

    # x = torch.randn(2, 3, 32, 32)
    # net = resnet18_cifar_aux(num_classes=100)
    # logit, ss_logits = net(x)
    # print(logit.size())

    # net = resnet34_cifar_aux(num_classes=1000)
    # from utils.utils import cal_param_size, cal_multi_adds
