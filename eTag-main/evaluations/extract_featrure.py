...
def extract_features(model, data_loader, print_freq=1, metric=None):
    ...
    for i, data in enumerate(data_loader, 0):
        ...
        inputs = imgs.cuda() 
        with torch.no_grad():
            feat = model(inputs, is_feat=True)
            for i in range(len(feat)):
                feat[i] = feat[i].cpu().numpy()
        if feats == []:
            feats = feat
            labels = label
        else:
            for i in range(len(feat)):
                feats[i] = np.vstack((feats[i], feat[i]))
...
def extract_features_logits(model, data_loader, print_freq=1, metric=None):
    model = model.cuda()
    model.eval()
    # batch_time = AverageMeter()
    # data_time = AverageMeter()
...
def extract_features_classification_aug(model, data_loader, print_freq=1, metric=None):
    model = model.cuda()
    model.eval()
    # batch_time = AverageMeter()
    # data_time = AverageMeter()
    ...
    for i, data in enumerate(data_loader, 0):
        ...
        with torch.no_grad():
            ...
            outputs = model.backbone.embed(outputs)
            outputs = outputs.cpu().numpy()
        # print(outputs.size())
        # for output, pid in zip(outputs, pids):
        if features == []:
